...
def _calculate_geometric_confidence(ratios: List[float], sequence_length: int, base_confidence: float = 0.95) -> float:
    """
...
def predict_next_in_sequence(sequence: List[float], reasoning_chain: Optional[ReasoningChain] = None, *, rtol: float = 0.3, atol: float = 1e-8) -> Optional[float]:
    ...
    if len(diffs) > 0 and np.allclose(diffs, diffs[0], rtol=rtol, atol=atol):
        ...
        if reasoning_chain:
            reasoning_chain.add_step(stage=stage, description=description, result=result, confidence=confidence, evidence=evidence, assumptions=assumptions)
        return result
    ...
    description = f"No simple arithmetic or geometric pattern found for sequence: {sequence}"
    if reasoning_chain:
        reasoning_chain.add_step(stage=stage, description=description, result=None, confidence=0.0)
    return None
...
def find_pattern_description(sequence: List[float], reasoning_chain: Optional[ReasoningChain] = None, *, rtol: float = 0.3, atol: float = 1e-8) -> str:
    ...
    if len(sequence) < 2:
        ...
        if reasoning_chain:
            reasoning_chain.add_step(stage=stage, description=description, result=result_str, confidence=0.0)
        return result_str
    ...
    if len(diffs) > 0 and np.allclose(diffs, diffs[0], rtol=rtol, atol=atol):
        ...
        confidence = _calculate_arithmetic_confidence(diffs, len(sequence), base_confidence=0.9)
        evidence = f"Common difference {diffs[0]} found in {diffs}. Confidence based on pattern quality and data sufficiency."
        if reasoning_chain:
            reasoning_chain.add_step(stage=stage, description=description, result=result_str, confidence=confidence, evidence=evidence, assumptions=assumptions)
        return result_str
    ...
    if all(s != 0 for s in sequence):
        ...
        if len(ratios) > 0 and np.allclose(ratios, ratios[0], rtol=rtol, atol=atol):
            ...
            # Use higher base confidence for pattern description than prediction
            confidence = _calculate_geometric_confidence(ratios, len(sequence), base_confidence=0.9)
            evidence = f"Common ratio {ratios[0]} found in {ratios}. Confidence based on pattern quality and data sufficiency."
            if reasoning_chain:
                reasoning_chain.add_step(stage=stage, description=description, result=result_str, confidence=confidence, evidence=evidence, assumptions=assumptions)
            return result_str
    ...
    if reasoning_chain:
        reasoning_chain.add_step(stage=stage, description=description, result=result_str, confidence=confidence)
    return result_str
